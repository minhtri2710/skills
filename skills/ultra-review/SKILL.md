...
Resolve `scripts/create_ultra_review_report.py` relative to this `SKILL.md` when
the current working directory is elsewhere. Use the script's `report_path`;
never improvise or overwrite it. Replace every `TODO`. If scouts submitted no
candidates, state `No candidates reported.` After writing, print the report path
and full content.
