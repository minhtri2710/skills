...
description: "Control Herdr, a terminal multiplexer for coding agents, and run bounded delivery inside it: pane and agent operations, intake and ownership, one shared checkout worked sequentially, foreground agent lifecycle, exact-head independent review, Human-gate routing, evidence handoff, and safe closeout. Use whenever the user explicitly mentions Herdr — to inspect or control panes, tabs, workspaces, commands, or another agent, to implement a change with another agent, to run an implementation-to-review pipeline, or to monitor a bounded delivery. Do not use merely because a task could benefit from a background terminal, delegation, or parallel work. Requires HERDR_ENV=1."
---
...
- **Delivery:** a source change needs bounded ownership, an implementation agent, implementation-to-review handoff, or explicit bounded monitoring/closeout. Read `references/intake-policy.md` first, then `references/coordinator-policy.md`, before mutation.
...
A delivery is one sequential loop in one checkout: plan, implement, review, and on a `FAIL` repair and review again. Stages never overlap, because they share a working tree. Follow this order:
...
2. **Topology:** preserve the caller's focus and working directory. The delivery runs in the caller's existing checkout: one branch, one writer, no second working tree. Keep one delivery in flight per project and serialize the rest. Independence comes from sequencing and from staffing the reviewer on a different agent kind than the implementation agent when one is available.
...
6. **Review:** obtain an independent verdict on the exact implementation head, in the same checkout with the implementation agent idle and the tree quiet at that head. A settled agent state is not acceptance.
7. **Repair:** route in-scope review findings to the implementation agent. Every resulting head needs fresh evidence and fresh review. The loop is bounded: when the repair cap (default two cycles, `repair-cap` in the project config) is reached without a `PASS`, stop, classify each finding as recurring or new, and route the Human gate or `SCOPE_REOPEN` instead of another cycle.
...
9. **Gate and close:** stop for Human-owned decisions, including push, PR mutation, merge, and deploy; accept only after claim-shaped checks and exact-head review; close only resources created by this run when cleanup is safe, including tearing down this issue's implementation and reviewer agents before the flight slot moves to the next issue.
...
Require the implementation evidence report before review and the reviewer verdict before acceptance. If output is trapped on an alternate screen, use the bounded fallback in `references/herdr-cli.md`: ask for a complete Markdown report in a temporary location and read it directly.
