...
- with explicit Human permission for that occasion, append to the gate ledger the one row the Human instructs, verbatim, by running the ledger script with `--writer supervisor-as-hands`, and only after the Lead's own runtime refused that append (`lead.md`, "Gates and closeout"). The permission is per occasion, recorded verbatim in the notebook with who typed it; it transfers no authority over the ledger, and the Supervisor never appends a row on its own reading and never rewrites or deletes one;
