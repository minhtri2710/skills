...
Reports arrive as prompts: a Peer's report wakes the Lead mid-turn or opens a new Lead turn, so the Lead ends its turn after dispatching and after each wake, and before ending any turn runs `scripts/roster.py` once to reconcile live Peers with reports received (`lead.md`, "Lifecycle and reports"). As part of that same once-per-wake reconcile, the Lead runs `scripts/roster.py --stalled` once when the required peer paths and previous sample are available. This is not polling: a check run once during a wake the Lead already has is not polling; polling is scheduling repeated self-wakes. Use `scripts/roster.py` for this roster/Discovery pass (`--workspace <id>` when scoped), consulting raw `herdr agent list` only if its compact view omitted a required field. Do not poll `herdr agent list`, sleep in a loop, re-issue status commands, or block on a Peer with any wait. The no-poll rule scopes to LLM seats burning turns; a non-LLM watcher burns no turns, so process watchers are not barred by that rule. A watcher reads state and pings only the Lead with a diagnosis line; it never touches the Peer, and the Lead alone executes recovery. Herdr-integration wiring for such a watcher is an external dependency, not doctrine scope.
