...
- The config may add stricter local preferences; it may not weaken this workflow's safety boundaries. A `lane-defaults` entry raises a lane floor and never lowers a lane below what the hard-gate classes (`lead.md`, "Intake") require. `always-gate` adds Human gates; no key removes one, authorizes an external write, or skips review. `worker-cap` bounds how many Engineers share the tree at once; it never permits overlapping owned paths or an agent that commits. When `reviewer-fallback` equals `engineer-kind`, the Lead reports a config conflict in the intake record rather than accepting it silently; if that fallback is ever used, independence requires a distinct model pinned through `reviewer-fallback-args`, and without one the fallback is unusable and conflicting, not independent. The `engineer-fallback` and `engineer-fallback-args` keys play for a died Engineer the role `reviewer-fallback` plays for a Reviewer: they pre-choose the kind and the pinned model the Lead restaffs on when the primary Engineer's model is cooling down or otherwise refused. This pair carries no independence rule — a replacement Engineer only has to stay inside its owned paths, which the partition already enforces — so it may repeat `engineer-kind` and needs only a model that is actually reachable.
