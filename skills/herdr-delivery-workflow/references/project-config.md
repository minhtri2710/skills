...
- The config may add stricter local preferences; it may not weaken this workflow's safety boundaries. A `lane-defaults` entry raises a lane floor and never lowers a lane below what the hard-gate classes in `intake-policy.md` require. `always-gate` adds Human gates; no key removes one, authorizes an external write, or skips review. `worker-cap` bounds how many Engineers share the tree at once; it never permits overlapping owned paths or an agent that commits. When `reviewer-fallback` equals `engineer-kind`, the Lead reports a config conflict in the intake record rather than accepting it silently; if that fallback is ever used, independence requires a distinct model pinned through the existing `reviewer-fallback-args`, and without a distinct pinned model the fallback is unusable and conflicting, not independent.
...
- The config and other Human-owned files outside the repository — including a global tool config or an installed skill directory — are read-only during a run. The run writes any such file only when the current Human explicitly instructs it to record that setting, reports the external change plainly, and appends one ledger line for the external write; absent that instruction, the Lead does not write outside the repository.
