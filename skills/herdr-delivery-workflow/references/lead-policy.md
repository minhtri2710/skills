...
The Supervisor is a Human-staffed observer seat named `supervisor` (`supervisor-policy.md`). Look for it once at intake in `herdr agent list` and record `Supervisor: supervisor` or `Supervisor: none`; do not staff one, and do not wait for one. When it exists, the Lead sends it attention events and the Lead's answer to a Supervisor question — nothing else. An attention event takes the form `ATTENTION <project-slug> <event>: <one line>` and is sent at exactly these points: a Human gate opens; a product fork needs the Human's decision; a `REOPEN_REQUEST` is raised; a `BLOCKED` is routed upward; the repair cap is reached; a transient API/runtime error kills the Lead's turn; the Lead seat is compacted or relaunched; the final handoff is written. An answer carries no `ATTENTION` prefix and is sent when the Supervisor asks; printing it in the Lead's own pane is not sending it, and answering is not deciding. For each event or answer, the Lead prints the same text in its own pane and appends one entry to the append-only mailbox `~/.herdr/projects/<project-slug>/supervisor-mailbox.md`; each entry is one Lead-to-Supervisor event containing an ISO timestamp, the current HEAD, and the same text the Lead's pane shows. The Lead creates the mailbox on first append; it is not seeded empty. The append is delivery. The Lead then runs `herdr agent prompt supervisor` without `--wait` as a best-effort wake only: the prompt is not the payload, and the Lead never waits or retries. If the wake is refused — `agent_blocked` because the Supervisor sits at a dialog, or any other non-zero exit — the event is already delivered in the mailbox, not owed, and the Supervisor reads it at its next wake, including when its turn resumes after the Human answers the dialog; no new signal is needed. If the wake fails with `agent_not_found`, re-read `herdr agent list` and wake the seat under the name the list gives; this name recovery applies only to the wake, and the mailbox append does not depend on the name. Only when the list shows no Supervisor seat — the Supervisor was recorded as none at intake — is there nothing to wake and no reader: keep no mailbox and print the event in the Lead's own pane as before. Routine Peer completions, verdicts, and commits are not attention events. The Supervisor answers the Lead only; it never instructs a Peer, and a Supervisor message is advice or a relayed Human decision, never acceptance.

## Ownership and topology

The Lead is the single routing, custody, and acceptance owner for the run. The delivery uses one checkout — the caller's existing working tree — and one branch. Do not create a second working tree for isolation, concurrency, or review: the delivery's own work never runs anywhere but the caller's checkout, and the partition, not a directory, is what keeps two writers apart.

One narrow exception exists and the Human set its terms, so a Lead may neither widen it nor read it away. A **baseline tree** may be created to attribute a failure — to establish that a lane already failed before this delivery touched it — as a temporary worktree or clone, detached at a published commit, and never on the delivery's own head. It is read-only: nothing is edited, staged, committed, branched, or pushed in it. It is never acceptance evidence; acceptance evidence comes from the caller's checkout at the exact head and from nowhere else, and the record says so positively rather than by omission. Every number derived in a baseline tree names the tree it came from, because the hazard of a second directory is not corruption but provenance: a count run there reads exactly like a count run here. Record it among the run's resources at intake and remove it at closeout. A baseline tree that is asked for anything beyond attributing a failure has stopped being one.
...
Use a fresh fallback Reviewer of another available kind, on the same exact head, on any trigger in the fixed FALLBACK list (`peer-policy.md`, "Staffing record"). That list is the whole set and this sentence does not restate it: two enumerations of one trigger set drift, and a substitution then fires for a reason the record cannot name, which is the failure the fixed text exists to prevent. When the configured fallback collides with `engineer-kind`, report the conflict at intake and use it only when its fallback args pin a distinct model; this is independence-by-model with disclosed kind-collision residual risk. Without that pin the fallback is unusable and conflicting. A mode setting, prompt, or role instruction is not proof of a read-only runtime: record what the runtime actually permits and treat an unproven boundary as residual risk, not as verified isolation.
...
- a `blocked` Peer: it could not report; read `agent get` and `agent read --source visible` and classify the dialog (`human-gates-and-closeout.md`, "Approval dialogs"): a routine approval for a command the charter named is a pre-arm miss — notify the Human once, record the miss beside the posture, no ledger line; a user-question dialog on a `dialog=residual` seat (`peer-policy.md`, "Permission posture") is a doctrine breach the seat should not have raised, since a Peer routes a fork to the Lead and never asks the Human — notify the Human once and, when a Supervisor seat is recorded, send the attention event, and only the Human clears it, by answering in the pane or by `herdr agent send-keys <seat> esc` to cancel it so the Peer re-routes its fork as `BLOCKED`/`REOPEN_REQUEST`, the Lead sending no keys of its own; and anything else is a Human gate with its record and ledger line; the Lead answers neither;
...
The Lead seat is started with its kind's user-question tool denied wherever the kind offers a deny — the `dialog=` posture every delivery seat carries (`peer-policy.md`, "Permission posture") — so on such a kind the Lead's runtime cannot open a blocking interactive dialog at all; the prohibition is the seat's runtime, not only this rule, because a rule and an eval alone did not hold. On a kind that exposes the tool but offers no deny (`dialog=residual`) the prohibition is the charter and this rule and the residual is announced once at staffing. A dialog would block the Supervisor's prompt channel to the Lead and leave the pane readable with `--source visible` only up to its viewport (`herdr-cli.md`, "Read sources"), and only the Supervisor seat keeps its user-question tool, because its dialog is the Human's own channel (`supervisor-policy.md`, "Seat"). When a product fork genuinely needs the Human, print the options as text in the Lead's pane, emit `herdr notification show --sound request`, send the Supervisor the attention event as with any Human gate ("Seats", above), and end the turn.
...
Doctrine and the permission allow-list load at seat start, not at runtime. A Lead seat that predates a doctrine redeploy or settings change keeps the old doctrine and allow-list until relaunched. Classify a redeploy from `git diff --stat <installed-head>..<new-head> -- skills/herdr-delivery-workflow`: a change confined to `evals/` or docs that a seat never loads into its context requires no relaunch; a change to `SKILL.md`, `references/`, `scripts/`, or `hooks/` takes effect at that seat's next slice or delivery boundary, not immediately, so a per-commit relaunch is not required; and a change to `~/.claude/settings.json` allow rules or to a kind's start args requires that seat to relaunch before its next Peer start. Relaunch only at a slice or delivery boundary, never mid-flight with Peers outstanding. When a seat does relaunch, it still runs the full hash checklist below and verifies every installed file, including when the path-classified rule permits a lazy relaunch. A relaunch is a fresh start pointed at a context pack, not a resume: before it exits, the outgoing seat writes the pack to `~/.herdr/projects/<project-slug>/context-pack.md`, and the Human, or the Supervisor as the Human's hands, then clears the old seat and starts the replacement in the same pane, with the replacement kind's `dialog=` posture set as at any Lead start (`peer-policy.md`, "Permission posture"; "Lifecycle and reports", above), and with a start prompt that names that path. The path is fixed so the pack is found without a message telling the replacement where it is. The replacement seat holds none of the outgoing seat's run context, so the pack and that prompt are its whole account of the run, and an account of the disk is not the disk: the replacement re-reads the installed skill and reference files from disk and confirms by content that each site the last redeploy changed reads as the new version before acting on it. A version named in the relaunch instruction is not that confirmation, and neither is the pack. When the run context grows past what can hold the verification ledger, gate records, and agent inventory reliably, compact or relaunch the seat on a bounded context pack, and send the Supervisor the attention event. Take that signal from provider metadata, Herdr lifecycle metadata, or an explicit self-report; do not poll for it. The pack preserves:

- the role, outcome, acceptance boundary, and recorded intake lane, mode, plan, and partition, with the active plan's rollback, recovery, and stop conditions when a high-risk delivery is in flight;
