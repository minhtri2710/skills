...
The Supervisor is a Human-staffed observer seat named `supervisor` (`supervisor-policy.md`). Look for it once at intake in `herdr agent list` and record `Supervisor: supervisor` or `Supervisor: none`; do not staff one, and do not wait for one. When it exists, the Lead sends it attention events — nothing else — with `herdr agent prompt supervisor "ATTENTION <project-slug> <event>: <one line>"`, no `--wait`, at exactly these points: a Human gate opens; a `REOPEN_REQUEST` is raised; a `BLOCKED` is routed upward; the repair cap is reached; the Lead seat is compacted or relaunched; the final handoff is written. Routine Peer completions, verdicts, and commits are not attention events. The Supervisor answers the Lead only; it never instructs a Peer, and a Supervisor message is advice or a relayed Human decision, never acceptance.
...
Committing is custody, not repair. The Lead stages and commits what the Engineers wrote; it does not edit source content, and a fix it thinks is obvious still goes back to the owning Engineer.
...
- a `blocked` Peer: it could not report; read `agent get` and `agent read`, then route the Human decision or the concrete blocker;
- a Peer missing from the list: it died; preserve whatever its pane still shows, record the replacement, and restaff the same disposition with the same charter — an Engineer keeps its scope and owned paths and inherits the dirty tree as it stands, a Reviewer is the pre-chosen fallback kind on the same exact head.
...
The Lead may route work, decide the partition, stage and commit at a quiesce point, classify findings, verify evidence, and decide whether the stated acceptance boundary is met. The Lead does not write source content: after a review finding it sends the bounded fix to the owning Engineer. Scope, architecture, dependency, security, external-effect, and irreversible changes require the appropriate Human decision before crossing the boundary.
