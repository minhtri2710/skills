...
For a project whose root predates this rule and still holds loose per-run files, the project's Lead applies this cleanup at its next closeout, not as a separate pass: move each such file into `runs/<issue-id>/`, or remove it only after proving by content that it is redundant to a retained record, using identical `git hash-object` results or equal `git cat-file` output. The enumerated coordination set and `gates.legacy.md` stay at the root; `config.md` and the live coordination files — the ledger, notebook, mailbox, and context pack — are never touched.
