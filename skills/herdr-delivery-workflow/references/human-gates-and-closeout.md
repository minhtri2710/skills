...
```text
<GATE-ID> | <ISO time> | <one-line finding> | <branch>@<exact head> | options A/B/C | status=open|resolved:<choice> | channel=direct-seat-pane:<typed|dialog>|supervisor-relay:<typed|dialog> | quote="<literal Human words>" | writer=<seat that appended the line, present only when it is not the Lead> | record=timely|reconstruction source=<source> | push=<push-base>..<push-head> count=<non-zero commit count> boundary-check="<the subtraction's output, empty when no path fell outside the boundary>"
...
The Lead writes the ledger. When the Lead's own runtime refuses the append, that refusal is a constraint on the seat and never a Human ruling ("Attribution"): record it as `Runtime denial: <command> — <effect on this run>` and retry in the narrowest allowed shape before treating it as blocking. If no shape lands and the row must carry a verbatim Human quote, state the exact row and ask; the Human may instruct the Supervisor to append it as the Human's hands (`supervisor-policy.md`, "Authority"). The Supervisor then appends only the row the Human instructed, verbatim, marks it `writer=supervisor-as-hands`, and records the instruction in the notebook with who typed it; it gains no authority over the ledger and never appends a row on its own reading. Who typed the row does not change `record=`: it is `timely` when the line was appended as the gate resolved and `reconstruction` otherwise.
