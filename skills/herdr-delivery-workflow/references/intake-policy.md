...
`partitioned` mode staffs one or more Engineers. `solo-Lead` mode is declared at intake, is not a config key or fallback, staffs zero Engineers for the whole run, and records the Lead-owned scope; a run that staffed an Engineer which then died restaffs or blocks, never entering solo-Lead.

Declaring `solo-Lead` needs a reason on the evidence, and the test is whether an Engineer would exercise judgement or transcribe someone else's. Write the charter's outcome line before choosing the mode. If the Lead can state what the scope must achieve without stating the edits that achieve it, an Engineer brings an independent reading of the problem and the run is `partitioned`. If the Lead cannot describe the work except by describing the diff, the charter would restate it, the Engineer would type the Lead's design rather than judge it, and the independence is nominal — that is `solo-Lead`, and the Reviewer is the second reader the mode does not supply. One condition is independently sufficient for `solo-Lead` and it is exhaustive: the changed artifact is the Lead's own coordination record — the gate ledger, the context pack, the intake record, the handoff. No Engineer can own those. Nothing else qualifies. A decision that feels like it needs the Lead's own authority but changes none of those files still goes to the primary test, because a branch a Lead can widen by calling a decision important is the escape this paragraph exists to close. Record which branch the test took and the evidence for it. A criterion that never selects `partitioned` is not being applied: a Lead whose runs are all `solo-Lead` has a finding about its own intake, not a run of unusually small slices. One scope is the base case and needs no justification. In `partitioned` mode, split into several scopes only when all of the following hold on the evidence, not on the issue title:
