...
- the agent kind running the review and the kind of every Engineer whose scope is in the head;
...
A `FAIL` is routed by the Lead to the Engineer that owns the affected paths for a bounded fix; a finding that spans scopes is the Lead's to sequence, not the Reviewer's to assign. If a finding increases scope, architecture, ownership, or proof risk, return `REOPEN_REQUEST` before implementation continues. The Lead never repairs source code from the review. Every fix creates a new exact head and requires a new review.
