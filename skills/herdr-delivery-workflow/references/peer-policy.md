...
A Peer that stops at an approval dialog for `git diff` or the test command is the most common stall in this workflow, and the Lead cannot answer that dialog. So the posture is set once for each seat, at that seat's `herdr agent start`, through the kind's native arguments after `--`, and recorded beside the seat and kind. Kinds differ in what they offer, and the flags change between releases, so learn them from the kind's own `--help` at staffing rather than from memory. The help is what settles whether the kind has a command allowlist, a scoped permission mode, or both, so read it for each of them before recording any posture: until it has shown there is none, `allowlisted` is still live. The project config supplies recorded argument values (`engineer-args`, `reviewer-args`, `reviewer-fallback-args`) when present; it never answers what the kind offers, and silence there about a kind is not evidence that the kind offers nothing finer. Choose the finest posture the kind offers:

- `allowlisted` — the kind accepts a command allowlist: pre-authorize exactly the read-only and verify commands the charter names plus `herdr agent prompt <lead-name>`, and for a Reviewer or Architect add a mode that withholds writes to the tree when the kind has one. Those are two axes, not one flag, and a kind may offer either without the other: a mode that withholds tree writes arms no command, so a seat started under it alone still stops at its first `git rev-parse` — the stall this posture exists to prevent. Record `allowlisted` for the arming axis only. A kind offering the write-withholding mode and nothing else is unarmed, and its posture is whatever the arming axis actually is; say which axes were passed, because "recorded as allowlisted" otherwise reads as a claim about both.
