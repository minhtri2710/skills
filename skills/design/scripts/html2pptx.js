...
const { toFileHref, toLocalPath } = require('./deck_helpers.cjs');
...
function collectTextBoxPositionWarnings(slideData, bodyDimensions) {
  const warnings = [];
  ...
  for (const el of slideData.elements) {
    if (['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'list'].includes(el.type)) {
      ...
      if (fontSize > 12 && distanceFromBottom < minBottomMargin) {
        ...
        warnings.push(
          `Text box "${textPrefix}" ends within ${minBottomMargin}" of the bottom edge ` +
          `(${distanceFromBottom.toFixed(2)}" from bottom). Review this manually if the layout is intentional.`
        );
      }
    }
  }
...
async function addBackground(slideData, targetSlide, tmpDir) {
  if (slideData.background.type === 'image' && slideData.background.path) {
    let imagePath = toLocalPath(slideData.background.path);
...
function addElements(slideData, targetSlide, pres) {
  for (const el of slideData.elements) {
    if (el.type === 'image') {
      let imagePath = toLocalPath(el.src);
...
async function extractSlideData(page) {
  return await page.evaluate((supportedTextTags, decorateListRunsSource) => {
    const PT_PER_PX = 0.75;
    const PX_PER_IN = 96;
    const decorateListRuns = new Function(`return (${decorateListRunsSource});`)();
    ...
    const elements = [];
    const placeholders = [];
    const textTags = supportedTextTags;
    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      if (el.tagName === 'UL' || el.tagName === 'OL') {
        ...
        const liElements = getDirectChildListItems(el);
        const items = [];
        ...
        liElements.forEach((li, idx) => {
          ...
          const runs = decorateListRuns({
            listTagName: el.tagName,
            index: idx,
            textIndent,
            runs: parseInlineFormatting(li, { breakLine: false })
          });
          if (runs.length > 0) {
            if (!isLast) {
  ...
  }, SUPPORTED_TEXT_TAGS, decorateListRuns.toString());
}
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    ...
    try {
      ...
      await page.goto(toFileHref(filePath));
      await waitForLayoutReady(page);

      const initialBodyDimensions = await getBodyDimensions(page);

      await page.setViewportSize({
        width: Math.round(initialBodyDimensions.width),
        height: Math.round(initialBodyDimensions.height)
      });
      await waitForLayoutReady(page);
    ...
    const textBoxPositionWarnings = collectTextBoxPositionWarnings(slideData, bodyDimensions);
    if (textBoxPositionWarnings.length > 0) {
      for (const warning of textBoxPositionWarnings) {
        console.warn(`${htmlFile}: ${warning}`);
      }
    }
