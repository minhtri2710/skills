...
- **Delivery Isolation**: Memory failures or missing indexes must never block ticket delivery, reopen closed delivery, alter review verdicts, or resolve Human Gates.
...
- **Internal Helper Identity**: `beo-recall` is a strictly internal helper identity used by `beo_recall.py` to call read-only qmd retrieval endpoints (`qmd.query` / `qmd.get`). It is NOT a loadable owner in `pipeline.json`, cannot claim tickets, and holds zero delivery, validation, execution, or mutation authority.

Default helper:
Recall helper: `beo-reference` -> `scripts/beo_recall.py --issue <issue-id>`.
After claim, when a durable artifact is needed, add `--write-summary`.
...
Post-accept learnings are written by `beo-learn` through `beo_memory_write.py` only after a recorded review verdict or debug return plus a valid `learning_candidate`; the helper uses Obsidian CLI when configured and falls back to local markdown.
...
- **Dynamic Vault Resolution**: The target vault path and collection name are resolved from:
  ...
  - `BEO_QMD_COLLECTION` (falls back to local markdown fallback if none are defined)
- **Command Contract**: `command-contracts.json` defines the contracted CLI shapes that scripts **must** load via `CommandAdapter`. Setup must verify the installed dialect before Obsidian-backed writes are enabled.
...
- **Setup & Indexing**: `beo_setup.py --configure-memory` and `beo_setup.py --refresh-memory-index` are the only setup-owned memory maintenance entry points and require explicit user authorization before qmd/Obsidian writes; exact command contracts live in `command-contracts.json`.
