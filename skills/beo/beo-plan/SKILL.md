...
10. Write the smallest current `version: 1` `TICKET.yaml` for quick, standard, or strict mode. A child bead whose ticket was pre-written under step 8 keeps that ticket; correct it in place if a `validation_failed` finding requires it, rather than authoring a new one.
...
- `.beads/artifacts/<issue-id>/TICKET.yaml` when the claimed issue is atomic, and `.beads/artifacts/<child-issue-id>/TICKET.yaml` for every pre-written child atomic bead at `decomposition_recorded`
