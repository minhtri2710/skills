...
8. On re-entry after `plan_validated`, create child atomic beads directly from the validated `PLAN.md` proposed atomic beads, dependency edges, and a parent summary comment that references `PLAN.md`, then stop. Each child bead description must be self-contained: include the implementation request/context, done criteria, expected scope, verification guidance, dependencies/blockers, suggested mode/risk notes, and atomicity rationale needed to plan/validate/execute that child without rereading the parent `PLAN.md`. Do not use “see parent PLAN.md” as required implementation context. Do not reinterpret parent requirements outside the validated plan.
