...
function statusScriptContent() {
  return [
    ...
    '',
    "  const requiredReads = ['AGENTS.md']",
    '  const conditionalReads = []',
    '  if (existsSync(criticalPatternsPath)) {',
    "    conditionalReads.push({",
    "      path: '.beads/critical-patterns.md',",
    "      read_when:",
    "        'beo-references -> learning.md records repo-policy startup-critical designation OR applicability matches active feature',",
    '      mandatory: false,',
    '    })',
    '  }',
    '  if (stateJson) {',
    "    conditionalReads.push({ path: '.beads/STATE.json', read_when: 'present', mandatory: true })",
    '  }',
    '  if (handoff) {',
    "    conditionalReads.push({",
    "      path: '.beads/HANDOFF.json',",
    "      read_when: 'present and fresh against live state',",
    '      mandatory: false,',
    '    })',
    '  }',
    '',
    ...
    "    `next_reads.required: ${status.next_reads.required.join(', ') || 'none'}` ,",
    "    `next_reads.conditional: ${status.next_reads.conditional.length ? status.next_reads.conditional.map((entry) => `${entry.path} [mandatory=${entry.mandatory}; when=${entry.read_when}]`).join('; ') : 'none'}` ,",
    '  ]',
