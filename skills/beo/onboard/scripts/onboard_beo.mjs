...
function statusScriptContent() {
  ...
  return [
    ...
    "  const approvalStatus = stateJson?.approval_ref ? 'current' : 'missing'",
    "  const handoffFreshness = handoff ? 'unknown' : 'none'",
    ...
    '    recommended_next: {',
    "      owner: stateJson?.current_owner ?? 'beo-route',",
    "      reason: stateJson?.current_owner ? 'follow canonical live owner unless live artifacts contradict it' : 'owner missing or scout found incomplete state',",
    '    },',
    '    warnings: managedBlockCurrent ? [] : [\'managed AGENTS.md block missing or stale\'],',
    '  }',
    '}',
    '',
