...
Ordering notes:
1. Keep `learnings-pending` above `completed` so closed epics route to compounding first.
2. Treat `exploring` as the fallback when the context and planning-artifact states do not match.
