...
2. For epic/feature plan validation: validate `.beads/artifacts/<issue-id>/PLAN.md` against `beo-reference/templates/PLAN.template.md`. Confirm it references the parent bead, states goals and non-goals, gives overall completion criteria, records assumptions, defines scope boundaries, gives a verification strategy, and proposes atomic child beads with description, done criteria, expected scope, verification guidance, dependencies, and suggested mode/risk sufficient to author quick, standard, or strict child tickets. Emit `plan_validated`, `validation_failed`, or `user_review_needed`; then stop. Never write `PASS_EXECUTE` for parent plans, and never emit `plan_validated` while a blocking user/operator-owned open decision remains.
